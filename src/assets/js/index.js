...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Estoy vivo...", "author": "Crip" },
            { "message": "Hola, soy código.", "author": "Crip" },
            { "message": "Linux no es un sistema operativo, es un kernel.", "author": "Crip" }
        ];
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then().catch(err => {
            return this.shutdown(`error when searching for update :<br>${err.message}`);
        });
    ...
    async maintenanceCheck() {
        config.GetConfig().then(res => {
            ...
            console.error(e);
            return this.shutdown("No se ha detectado una conexión a internet,<br>por favor, inténtelo de nuevo más tarde.");
        })
    }
